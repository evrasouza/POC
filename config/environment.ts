...
function readCliOption(name: string): string | undefined {
  ...
  if (inlineArg) {
    const value = inlineArg.slice(prefix.length).trim();
...
function resolveOption(cliName: string, envName: string, defaultValue: string): string {
  return readCliOption(cliName) ?? readEnvironmentVariable(envName) ?? defaultValue;
}
